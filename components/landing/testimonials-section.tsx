...
export function TestimonialsSection() {
  return (
    <section id="testimonials" className="py-20 md:py-32 relative bg-background border-y border-border">
      <div className="absolute inset-0 bg-gradient-to-b from-secondary/10 dark:from-blue-950/40 to-transparent" />
          ...
          {testimonials.map((testimonial, index) => (
            ...
            >
              <Card className="bg-card/90 dark:bg-gradient-to-br dark:from-black/40 dark:to-blue-950/20 border border-border dark:border-blue-800/30 backdrop-blur-md p-8 h-full flex flex-col transition-all duration-300 hover:border-primary/30 dark:hover:border-blue-600/50 hover:bg-card dark:hover:bg-blue-950/30 shadow-md hover:shadow-lg dark:hover:shadow-xl dark:hover:shadow-blue-600/10">
                ...
                <p className="text-muted-foreground dark:text-blue-100/80 mb-6 text-base leading-relaxed flex-grow italic">
                  " {testimonial.quote} "
                </p>
                <div className="flex items-center mt-auto pt-4 border-t border-border dark:border-blue-800/20">
                  <Image
                    src={testimonial.image || "/placeholder-futuristic.svg"}
                    alt={testimonial.author}
                    width={44}
                    height={44}
                    className="rounded-full mr-4 border-2 border-border dark:border-blue-800/30"
                  />
                  <div>
                    <div className="font-semibold text-foreground dark:text-white tracking-tight">{testimonial.author}</div>
                    <div className="text-sm text-muted-foreground dark:text-blue-300/70 tracking-wide">{testimonial.position}</div>
                  </div>
                </div>
              </Card>
            </motion.div>
